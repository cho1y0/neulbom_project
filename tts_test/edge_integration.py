"""
노인 케어 시스템 통합 모듈
녹음 → STT → 분석 + LLM → TTS (다양한 목소리!) 파이프라인
"""
...
class ElderCareSystem:
    ...
    def __init__(self, use_tts=True, tts_engine="edge", tts_voice="sun-hi"):
        ...
        Args:
            use_tts: TTS 사용 여부
            tts_engine: "pyttsx3", "gtts", "edge" (다양한 목소리!)
            tts_voice: 목소리 선택 (edge 전용)
                      'sun-hi', 'soon-bok', 'in-joon', 'bong-jin' 등
        """
        print("="*60)
        print("🏥 노인 케어 시스템 초기화 중...")
        print("="*60)
        ...
        if use_tts:
            print(f"\n[4/4] TTS 초기화 ({tts_engine}, 목소리: {tts_voice})...")
            try:
                if tts_engine == "edge":
    ...
    def conversation_turn(self, save_recording=True):
        ...
        # 1. 녹음
        print("\n[1/4] 🎤 음성 녹음")
        print("말씀하세요. 침묵이 3초 지속되면 자동 종료됩니다.")
...
if __name__ == "__main__":
    ...
    system = ElderCareSystem(
        use_tts=True,
        tts_engine="edge",      # "edge", "pyttsx3", "gtts"
        tts_voice="sun-hi"      # 여성: sun-hi, soon-bok, ji-min
                                 # 남성: in-joon, bong-jin
    )
